...
test.describe('Session Storage vs Fresh Login Comparison', () => {
  test.describe('Session Storage Approach (reuses auth)', () => {
    test('worker assignment', async ({ dashboardPage }, testInfo) => {
      ...
      const accountIndex = testInfo.workerIndex % ACCOUNT_USERNAMES.length;

      expect(dashboardPage.username).toBe(ACCOUNT_USERNAMES[accountIndex]);
      console.log(`[SESSION] Worker ${relativeWorker} authenticated as ${dashboardPage.username}`);
    });

    test('maintains session', async ({ dashboardPage }, testInfo) => {
      const relativeWorker = getRelativeWorkerIndex(testInfo.workerIndex);
      await dashboardPage.assertUserIsLoggedIn();
      console.log(`[SESSION] Worker ${relativeWorker} (${dashboardPage.username}) session active`);
    });
    ...
    test('cart operations', async ({ dashboardPage }, testInfo) => {
      ...
      console.log(`[SESSION] Worker ${relativeWorker} (${dashboardPage.username}) cart: ${total}`);
    });
    ...
    test('add multiple items', async ({ dashboardPage }, testInfo) => {
      ...
      const total = await dashboardPage.getCartTotal();
      expect(total).not.toBe('Total: $0.00');
      console.log(`[SESSION] Worker ${relativeWorker} (${dashboardPage.username}) multiple items: ${total}`);
    });

    test('verify user role', async ({ dashboardPage }, testInfo) => {
      const relativeWorker = getRelativeWorkerIndex(testInfo.workerIndex);
      await dashboardPage.assertUserIsLoggedIn();
      console.log(`[SESSION] Worker ${relativeWorker} (${dashboardPage.username}) role verified`);
    });
  });
  ...
  test.describe('Fresh Login Approach (no auth reuse)', () => {
    test('worker assignment', async ({ freshDashboardPage }, testInfo) => {
      ...
      expect(freshDashboardPage.username).toBe(ACCOUNT_USERNAMES[accountIndex]);
      console.log(`[FRESH] Worker ${relativeWorker} authenticated as ${freshDashboardPage.username}`);
    });

    test('maintains session', async ({ freshDashboardPage }, testInfo) => {
      const relativeWorker = getRelativeWorkerIndex(testInfo.workerIndex);
      await freshDashboardPage.assertUserIsLoggedIn();
      console.log(`[FRESH] Worker ${relativeWorker} (${freshDashboardPage.username}) session active`);
    });
    ...
    test('cart operations', async ({ freshDashboardPage }, testInfo) => {
      ...
      expect(total).not.toBe('Total: $0.00');
      console.log(`[FRESH] Worker ${relativeWorker} (${freshDashboardPage.username}) cart: ${total}`);
    });
    ...
    test('add multiple items', async ({ freshDashboardPage }, testInfo) => {
      ...
      const total = await freshDashboardPage.getCartTotal();
      expect(total).not.toBe('Total: $0.00');
      console.log(`[FRESH] Worker ${relativeWorker} (${freshDashboardPage.username}) multiple items: ${total}`);
    });

    test('verify user role', async ({ freshDashboardPage }, testInfo) => {
      const relativeWorker = getRelativeWorkerIndex(testInfo.workerIndex);
      await freshDashboardPage.assertUserIsLoggedIn();
      console.log(`[FRESH] Worker ${relativeWorker} (${freshDashboardPage.username}) role verified`);
    });

    test('consistency check', async ({ freshDashboardPage }, testInfo) => {
      const relativeWorker = getRelativeWorkerIndex(testInfo.workerIndex);
      expect(freshDashboardPage).toBeDefined();
      console.log(`[FRESH] Worker ${relativeWorker} (${freshDashboardPage.username}) consistency verified`);
    });
  });
});